...
@Serializable
@SerialName("io.nats.jetstream.api.v1.stream_configuration")
public data class StreamConfiguration(
    ...
    /**
     * A list of subjects to consume, supports wildcards. Must be empty when a mirror is configured.
    ...
    /**
     * How many messages may be in a Stream, oldest messages will be removed if the Stream exceeds
     * this size. -1 for unlimited.
     */
    @SerialName("max_msgs")
    public val maxMsgs: Long,
    /**
     * For wildcard streams ensure that for every unique subject this many messages are kept - a per
     * subject retention limit
     */
    ...
    /**
     * When the Stream is managed by a Stream Template this identifies the template that manages the
     * Stream.
     */
     ...
     */
    public val discard: StreamDiscardType? = nats.jetstream.protocol.domain.StreamDiscardType.Old,
    /**
    ...
    /**
     * Placement directives to consider when placing replicas of this stream, random placement when
     * unset
     */
    public val placement: Placement? = null,
    /**
     * Maintains a 1:1 mirror of another stream with name matching this property.  When a mirror is
     * configured subjects and sources must be empty.
     */
     ...
     * be unsealed via configuration update. Can only be set on already created streams via the Update
     * API
     */
    public val `sealed`: Boolean? = false,
    /**
     * Restricts the ability to delete messages from a stream via the API. Cannot be changed once
     * set to true
     */
    @SerialName("deny_delete")
    public val denyDelete: Boolean? = false,
    /**
     * Restricts the ability to purge messages from a stream via the API. Cannot be change once set
     * to true
     */
    @SerialName("deny_purge")
    public val denyPurge: Boolean? = false,
    /**
     * Allows the use of the Nats-Rollup header to replace all contents of a stream, or subject in a
     * stream, with a single new message
     */
    ...
    /**
     * When discard policy is new and the stream is one with max messages per subject set, this will
     * apply the new behavior to every subject. Essentially turning discard new from maximum number of
     * subjects into maximum number of messages in a subject.
     */
